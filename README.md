...
2. **Advance to linked-object validation:** infer PsyQ object boundaries and
   ...
   **The falsifiable four-criterion bar defined in
   ...
   (`0x80032afc`–`0x80032b98`), and `VSYNC` (`0x800358f8`–`0x80035b08`), that
   meet criteria 1 and 2 but fail criterion 3 outright (both edges dense, zero
   padding bytes anywhere) — recorded as real negative results, not
   confirmed; all three sit inside the same tightly packed SPU-library region,
   suggestively (not yet proven) one dense object rather than several small
   ones. Every other `<name>_OBJ_*` run remains `candidate_only` until it
   independently passes the same checks. A bounded, ordered backlog toward
   object-name semantic identity (lowest priority; still open even for the
   confirmed objects) is defined in the
