...
| `docs/games/` (top level) | The multi-game registry (`index.md`); per-game content nests under `docs/games/<target>/`. |

A future second target adds its own `src/<target>/`, `config/<target>/`,
`tools/<target>/`, and `docs/games/<target>/`, without touching the shared
directories above.
...
- [Target revision](docs/games/ddr5thmix/revision-manifest.md)
