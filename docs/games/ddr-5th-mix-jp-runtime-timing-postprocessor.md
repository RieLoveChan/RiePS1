...
`0x80115e70 + 20·a0`, obtains the global value from `FUN_80099314`, and
post-processes it together with the record's word at offset zero and the
global bytes `0x800dfc8d` and `0x800dfc95`.

# Observed semantic behavior

- The argument `a0` selects one 20-byte record for the second term of the
  result. The delay-slot write of a record address before the call does not
  establish a callee parameter: `FUN_80099314` does not read `a0`.
...
- The byte at `0x800dfc8d` participates in a signed multiply path whose
  reciprocal-multiply constant implements division by 10.
- The byte at `0x800dfc95` participates in an unsigned multiply path whose
...
The units of the provider result and the meanings of the two global bytes
remain unresolved. The related initializer
...
identify the timing unit, recover original names, or
