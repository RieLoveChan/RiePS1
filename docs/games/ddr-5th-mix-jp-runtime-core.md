...
Total: sixteen functions and 4,060 selected bytes.
...
After those harness corrections, all sixteen functions matched without changing
...
`FUN_8002263c`, and `FUN_80020d24`, the same command matched all 4,060 bytes against the same
executable SHA-256.
The aggregate report remains ignored under `/build/`.

This proves sixteen independently placed function bodies. It does not prove an
