...
`FUN_8002a1b0` adds a 64-byte exact match in
`src/ddr5thmix/StateTransition.s`. It initializes the selected four-byte
entry through `FUN_800358f8(-1)` and returns the new result minus its prior
value. The verifier matched 64/64 bytes with GCC 14.2.0/binutils 2.43
against lawful executable SHA-256
`4e0308ca35000fe91bf0b468297125061efeb16198c27fd13c950003d94c4aee`; the
reference/built SHA-256 is `33f20c1be27c0ddf5376455433fc65e1fca4e871c99be142e3559aa72b7d51c3`.
This is a bounded semantic assembly reconstruction, not a claim about the
original source form.

# Timing mode setter

`FUN_8002a1f0` adds a 16-byte exact match in
`src/ddr5thmix/StateTransition.s`. It stores `272` into the halfword state
field at `0x800e38c6`. The verifier matched 16/16 bytes with GCC
14.2.0/binutils 2.43 against lawful executable SHA-256
`4e0308ca35000fe91bf0b468297125061efeb16198c27fd13c950003d94c4aee`; the
reference/built SHA-256 is `c5b1bd1f4f295bdf6949e8b3b199ae002dc84a72643fd51a6af281752b3139c5`.
This is a bounded semantic assembly reconstruction, not a claim about the
original source form.

# Timing mode setter

`FUN_8002a200` adds a 12-byte exact match in
`src/ddr5thmix/StateTransition.s`. It writes the supplied value into the
halfword state field at `0x800e38c6`. The verifier matched 12/12 bytes with
GCC 14.2.0/binutils 2.43 against lawful executable SHA-256
`4e0308ca35000fe91bf0b468297125061efeb16198c27fd13c950003d94c4aee`; the
reference/built SHA-256 is `9f37f6e83f03c04e6a30e8df8155cde2cd7d2342c806b5c1bc089daa51084ed5`.
This is a bounded semantic assembly reconstruction, not a claim about the
original source form.

# Timing identifier validator

`FUN_8002a20c` adds a 164-byte exact match in
`src/ddr5thmix/StateTransition.s`. It selects one of two state-dependent
halfword tables, returns `1` when the input identifier is found, and
otherwise returns `0`. The verifier matched 164/164 bytes with GCC
14.2.0/binutils 2.43 against lawful executable SHA-256
`4e0308ca35000fe91bf0b468297125061efeb16198c27fd13c950003d94c4aee`; the
reference/built SHA-256 is `56354d656666ae547184fbfd2e12178370fd3e768b0bdaafb04d0862bf9f76b3`.
This is a bounded semantic assembly reconstruction, not a claim about the
original source form.
...
# Timing quotient helper

`FUN_8002a3fc` adds a 60-byte exact match in
`src/ddr5thmix/StateTransition.s`. It converts the `FUN_8002a670` result
through a reciprocal multiply and returns the corrected value divided by
`128`. The verifier matched 60/60 bytes with GCC 14.2.0/binutils 2.43
against lawful executable SHA-256
`4e0308ca35000fe91bf0b468297125061efeb16198c27fd13c950003d94c4aee`; the
reference/built SHA-256 is `8930326abac20f44564319b27834950f210f067ccdb00681c3e3ebd53e3d1815`.
This is a bounded semantic assembly reconstruction, not a claim about the
original source form.
...
# Timing feedback updater

`FUN_8002a4c8` adds a 224-byte exact match in
`src/ddr5thmix/StateTransition.s`. It refreshes shared timing state,
computes a signed difference against the current target, dispatches
`FUN_8002a5a8` with an adjusted value, marks completion, and stores the
refreshed baseline. The verifier matched 224/224 bytes with GCC
14.2.0/binutils 2.43 against lawful executable SHA-256
`4e0308ca35000fe91bf0b468297125061efeb16198c27fd13c950003d94c4aee`; the
reference/built SHA-256 is `a15abab3059002268da1d4d55d9b126811ff66176936493728732736963d821d`.
This is a bounded semantic assembly reconstruction, not a claim about the
original source form.

# Timing scale dispatcher

`FUN_8002a5a8` adds a 200-byte exact match in
`src/ddr5thmix/StateTransition.s`. It stores the input baseline, scales it
through reciprocal division and a table byte, clamps the signed result to
`127`, and submits it to `FUN_8003164c`. The verifier matched 200/200 bytes
with GCC 14.2.0/binutils 2.43 against lawful executable SHA-256
`4e0308ca35000fe91bf0b468297125061efeb16198c27fd13c950003d94c4aee`; the
reference/built SHA-256 is `7621ac1d907157927e2820aeef301387c8aed480d98e9c9b95467b35cebd7605`.
This is a bounded semantic assembly reconstruction, not a claim about the
original source form.
