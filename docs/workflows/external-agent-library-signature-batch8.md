---
type: Workflow
title: External library-signature reconstruction — batch 8
description: A fixed 85-function package of 97–128-byte PsyQ-signature functions for isolated reconstruction and independent audit.
tags: [delegation, psyq, mips, reconstruction, byte-match, audit]
timestamp: 2026-07-29T17:00:00-04:00
---
...
functions, the second sliced sub-range of the 65-byte-and-larger backlog
(Batch 7 took the first, 65–96 bytes). Delivery by the external agent remains
...
- Base commit: `9f7a62b5f1ec7d244f85d4e8c2816e825c8bf11d`.
- Dedicated delivery branch:
  `agent/library-signature-reconstruction-batch8`.
...
- integer `size` is in the inclusive range 97–128 bytes.
...
it contains **85 unique functions / 9,612 bytes** and has SHA-256:

`96e24bd4ce4a16d446633bb5aad54e3a0391cbf2adce3e5fe892abf424f7390f`
...
if ($rows.Count -ne 85 -or
    ($rows | Measure-Object size -Sum).Sum -ne 9612 -or
    $sha256 -ne
        '96e24bd4ce4a16d446633bb5aad54e3a0391cbf2adce3e5fe892abf424f7390f') {
    throw 'Batch-8 inventory does not match the fixed contract.'
}
```

This deliberately excludes every function 96 bytes or smaller (already
reconstructed in Batches 3–7) and every function 129 bytes or larger (left for
...
- `src/ddr5thmix/LibrarySignatureBatch8.s` (new);
...
8. These functions (97–128 bytes, roughly 24–32 instructions) are the largest
   tier attempted mechanically so far and more likely to contain internal
   loops, multi-way branches, or nested control flow. Re-check every
   local-label target still falls strictly within `[0, size)` and that no two
   owned functions' instruction streams were transposed.
9. Add exactly one manifest entry for each of the 85 functions. Before
...
11. Promote exactly those 85 CSV rows to `confidence = verified` and
...
- 85/85 `byte_match: true`, with exactly 9,612 selected bytes;
- no duplicate manifest functions, sections, addresses, or promoted rows;
- all 85 manifest entries contain equal non-empty reference/built hashes;
...
inventory from 652 to **737 verified functions** (30.7% to **34.6%**), reduce
`library_signature` from 493 to **408**, reduce `decompiler_output_only` from
...
directive block, and its audit found a stray trailing blank line; Batches 6
and 7 were both clean passes on every gate. Before committing, fail the
package if any of these defects exists anywhere in the 85 owned entries:
...
- all 85 per-function reference/built hashes and match results;
